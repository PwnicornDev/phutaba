...
BEGIN { require "strings_" . BOARD_LANG . ".pl"; }
...
my %font        = (
    a => [
        4,
        [ 0, 2, 1, 1, 2, 1, 3, 2, 3, 5, 4, 6 ],
        [ 3, 3, 1, 3, 0, 4, 0, 5, 1, 6, 2, 6, 3, 5 ]
    ],
...
sub make_word {
    my %grammar = (
        ...
        C => [
            "b",  "c", "ch", "d", "f", "g",  "h",    "j",
            "k",  "l", "m",  "n", "p", "qu", "r",    "s",
            "sh", "t", "th", "v", "w", "y",  "s%P%", "%R%r",
            "%L%l"
        ],
        E => [
            "b", "c", "ch", "d", "f",  "g", "dg", "l", "m", "n",
            "p", "r", "s",  "t", "th", "v", "z"
        ],
        F => [
...
sub find_stylesheet_color {
        ...
        /(?:}|^)\s+\Q$selector\E\s*{[^}]*color:\s*([^;}]+?)\s*(?:;|}|^)/s )
    {
        ...
        if ( $color =~ /#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})/i ) {
            ...
            $color =~ /rgb\s*\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*\)/ )
        {
            return ( $1, $2, $3 );
        }
    }
...
sub make_image {
    ...
    $height = @pixels;
    $width = max( map { $_ ? scalar( @{$_} ) : 0 } @pixels );
...
sub draw_string {
    ...
    foreach my $char (@chars) {
        ...
        foreach my $stroke (@strokes) {
            ...
            ( $prev_x, $prev_y ) =
              transform_coords( shift @coords, shift @coords );
...
sub start_128_grey_gif {
    ...
    for ( my $i = 0 ; $i < 64 ; $i++ ) {
        print pack( 'CCC', $i * 2, $i * 2, $i * 2 );
    }
    for ( my $i = 64 ; $i < 128 ; $i++ ) {
        print pack( 'CCC', $i * 2 + 1, $i * 2 + 1, $i * 2 + 1 );
    }
    print pack( 'CvvvvCC', 0x2c, 0, 0, $width, $height, 0x00, 0x07 );
}
