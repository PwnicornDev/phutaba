...
<if $parent>
	...
	<if $admin>
		...
		<span onmouseover="TagToTip('postinfo_<var $num>', TITLE, '<const S_POSTINFO>', DELAY, 0, CLICKSTICKY, true, CLICKCLOSE, true)" onmouseout="UnTip()" onclick="highlight_all(this.innerHTML)">[<var dec_to_dot($ip)>]</span>
		</if>
